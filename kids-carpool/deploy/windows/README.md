...
- `JWT_SECRET` — 아래 PowerShell 한 줄로 무작위 문자열을 만들어 붙여넣는다.
  (PowerShell 5.1의 `Get-Random -Count`는 입력 배열의 고유 원소 개수만큼만 뽑기 때문에,
  `(48..57)+(97..122)` — 36개 문자 — 에 `-Count 48`을 주면 36자만 나온다. 아래처럼
  같은 문자를 중복 허용하며 48번 반복 추출해야 실제로 48자가 나온다.)

  ```powershell
  -join (1..48 | ForEach-Object { [char](Get-Random -InputObject ([char[]]'abcdefghijklmnopqrstuvwxyz0123456789')) })
  ```
...
> 서비스 기동 후, `C:\gachitayo\svc\logs\caddy-access.log`에 `/ws?token=...`의 실제 토큰 값이
> 그대로 남지 않고 `REDACTED`로 마스킹되는지 확인할 것.
...
- **SMB/파일 공유(445)도 인터넷에 노출하지 않는다.** 위 포트들과 마찬가지로 공유기/방화벽에서
  445를 외부로 포워딩하거나 열지 말 것.
...
- **백업은 서버 자신이 지울 수 없는 외부**(버전 관리가 되는 클라우드 동기화 폴더, NAS의
  ...
  구체적으로는 (1) 버전 관리가 되는 클라우드 저장소(이전 버전이 남아 있어 덮어써도 복구 가능)이거나,
  (2) 서버가 쓰는 계정과 별도의 자격 증명으로 "쓰기 전용(업로드만 가능, 삭제·덮어쓰기 불가)" 권한만
  부여된 업로드 방식이어야 한다 — 서버 계정 자체에 삭제/덮어쓰기 권한이 있으면 랜섬웨어나 침해 시
  오프사이트 사본도 함께 지워질 수 있다.
