    ...
    if (members.length > 0) {
      ...
      for (let dow = 1; dow <= 5; dow++) {
        ...
        await q(
          ...
          [gid, start, dow, driver.id, allKids],
        )
      }
...
api.post('/swap/:id/accept', async (req, res) => {
  ...
  if (!swap) return res.status(404).json({ error: '요청을 찾을 수 없어요' })
  const [to] = await q(
    'SELECT * FROM week_plans WHERE group_id = $1 AND week_start = $2 AND dow = $3',
    [gid, swap.week_start, swap.to_dow],
  )
  if (!to || to.driver_id !== req.user.id) return res.status(403).json({ error: '교환 상대 당번만 수락할 수 있어요' })
